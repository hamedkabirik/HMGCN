...
def predict_model(model, file_name, feature, A, node_types, eval_type, node_matching, device):
    """
    ...
    for _ in range(1):
        for iter_ in range(500):
            ...
            emb = model(feature, A, node_types)
            ...
            emb_true_first = torch.cat(emb_true_first).reshape(-1, args.out)
            emb_true_second = torch.cat(emb_true_second).reshape(-1, args.out)
            emb_false_first = torch.cat(emb_false_first).reshape(-1, args.out)
            emb_false_second = torch.cat(emb_false_second).reshape(-1, args.out)
            ...
            td = model(feature, A, node_types).detach().numpy()
            final_model = {}
            try:
